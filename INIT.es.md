...
> This is the Spanish version. The primary documentation is in English at [`INIT.md`](./INIT.md).
...
- Listar/reiniciar contenedores y enviar películas/series a descargas.
...
|---|---|---|
| id | UUID (PK) | Identificador único de la sesión de chat. |
| title | TEXT | Título generado automáticamente. |
| context_summary | TEXT | El resumen compactado de la conversación. |
...
| session_id | UUID (FK) | Relación con la tabla sessions. |
| role | VARCHAR | Rol del emisor: user, assistant, system o tool. |
| content | TEXT | Contenido en texto del mensaje. |
| image_url | TEXT | (Opcional) Ruta de la imagen adjunta. |
| tool_calls | JSON | (Opcional) Registro de las herramientas MCP ejecutadas. |
...
- Dos botones grandes y claros: **Autorizar** (Verde/Primario) y **Cancelar Acción** (Rojo/Secundario). Si se cancela, se informa al LLM de que el usuario abortó la operación.
